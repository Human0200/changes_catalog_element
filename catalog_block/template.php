...
<?if($arResult["ITEMS"]):?>
    ...
    <?endif;?>
        <?foreach($arResult["ITEMS"] as $arItem){
            ...
            <?$itemSideIcons = '';?>
            <?if (TSolution\Product\Common::isNeedActionIcons($arItem, $arParams)):?>
                <?$itemSideIcons = TSolution\Product\Common::getActionIcons([
                    'ITEM' => ($arCurrentOffer ?: $arItem),
                    'PARAMS' => $arParams,
                    'SHOW_FAVORITE' => $bShowFavorit,
                    'SHOW_COMPARE' => $bShowCompare,
                    'SIDE_CLASSES' => 'side-icons flexbox line-block--gap line-block--gap-12 ' . ($arCurrentOffer ? 'hide-600' : ''),
                    'ORIENT' => 'vertical',
                ]);?>
            <?endif;?>
            <div class="catalog-block__wrapper<?= $elementSliderClasses; ?> grid-list__item grid-list-border-outer <?=($arCurrentOffer ? 'has-offers' : '');?>"<?=($arItem['BIG_DATA'] ? ' data-bigdata data-rcm="'.$arItem['RCM_ID'].'"' : '')?> data-hovered="false">
                ...
                <div class="catalog-item catalog-block__item <?=$itemClass?>" id="<?=$arItem["strMainID"]?>">
                    <?if ($arItem['SKU']['PROPS']):?>
                        <?=TSolution\SKU::getTemplateWithJsonOffers($arItem["SKU"]["OFFERS"])?>

                        <?$bUseSelectOffer = true;?>
                    <?endif;?>
                    <div class="catalog-block__inner flexbox height-100 item-action-static-fill-svg" <?if ($bUseSchema):?>itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product"<?endif;?>>
                        <?if ($bUseSchema):?>
